  ...
  for (i = 0; frames[i]; i++)
    {
      ...
      gint width, height;
      g_autofree gchar *pixels_filename = NULL;
      g_autoptr(GFile) pixels_file = NULL;
      g_autoptr(GBytes) expected_pixels = NULL;
      ...
      width = g_key_file_get_integer (config_file, frame, "width", &error);
      g_assert_no_error (error);
      height = g_key_file_get_integer (config_file, frame, "height", &error);
      g_assert_no_error (error);
